...
class GAT(nn.Module):
    def __init__(
        ...
        for num_heads in num_heads_list[:-1]:
            self.layers.append(GAT_layer(temp_dim, hidden_dim, num_heads, bias, dropout, activation_att, activation, \
                                                                aggregation='cat', skip=skip, analysis_mode=analysis_mode))
            temp_dim = num_heads * hidden_dim
            
        self.layers.append(GAT_layer(temp_dim, output_dim, num_heads_list[-1], bias, dropout, activation_att, activation=activation_last, \
...
class ScGCN(nn.Module):
    def __init__(
        ...
        for _ in range(num_layers):
            self.hyb_layers.append(hybrid_layer(temp_dim, hidden_dim, config, bias, dropout, activation))
            temp_dim = hidden_dim * len(config)

        self.res_layer = GNN_layer(temp_dim, output_dim, bias, dropout, activation=None)
    ...
    def forward(self, data):
        ...
        for hyb_layer in self.hyb_layers:
            x = hyb_layer(x, data.gcn_mat, data.sct_mat)
...
class ScGCN_pre(nn.Module):
    def __init__(
        ...
        temp_dim = input_dim
        for _ in range(num_layers):
            self.hyb_layers.append(hybrid_layer_pre(temp_dim, hidden_dim, num_channels, bias, dropout, activation))
...
class ScGCN_rwg(nn.Module):
    def __init__(
        ...
        temp_dim = input_dim
        for _ in range(num_layers):
            self.rwg_layers.append(reweighting_layer(temp_dim, hidden_dim, num_heads, dropout, activation=activation_att))
            self.hyb_layers.append(hybrid_layer(temp_dim, hidden_dim, config, bias, dropout, activation))
            temp_dim = hidden_dim * len(config)
